...
class Avoider_v_1(base.PyGameWrapper):
    """
    ...
    def __init__(self, width=64, height=64, init_lives=3, obstacles=2):
    ...
    def getGameState(self):
        ...
        for i in range(len(obstacles_x)):
            if env_obs[obstacles_x[i] + 1, 0] == -1 or env_obs[obstacles_x[i] + 1, 0] > obstacles_y[i]:
    ...
    def step(self, dt):
        ...
        for i in range(len(self.list_of_obstacles)):  # , avoided in zip(self.list_of_obstacles, self.list_avoided):
            if self.list_of_obstacles[i].rect.center[1] >= self.height and not self.list_avoided[i]:
                self.list_of_obstacles[i].draw_collision(self.screen)
...
- with 2 fruits set for each a velocity (all the same)
- for each independant velocity
- check end conditions for all 3 independent but mak sure that no two are overlapped

Phase  2:  change  state-vector  description  to  have  per  line  distance  and  velocity  to  a  possibleobstacle (define constants for when there are no obstacles in the line) still discrete and solution
